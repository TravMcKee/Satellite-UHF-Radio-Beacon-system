...
figure
plot(abs(TOA_diff(:,1))*1000,'lineWidth',1);
hold on; grid minor; xlim([0 length(TOA_diff(:,1))]);
plot(abs(TOA_diff(:,2))*1000,'lineWidth',1);
plot(abs(TOA_diff(:,3))*1000,'lineWidth',1);
plot(abs(TOA_diff(:,4))*1000,'lineWidth',1);
plot(abs(TOA_diff(:,5))*1000,'lineWidth',1);
ax=gca;ax.FontSize=16;
...
for x=1:5
TOA_mean_NS(x) = mean(TOA_diff(:,x));
TOA_abs_mean_NS(x) = mean(abs(TOA_diff(:,x)));
TOA_StdDev_NS(x) = std(TOA_diff(:,x));
TOA_abs_StdDev_NS(x) = std(abs(TOA_diff(:,x)));
end
