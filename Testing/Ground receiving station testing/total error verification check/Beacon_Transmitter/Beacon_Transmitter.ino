/*
 * Beacon_Transmitter - Final verification check - determining total error between 2 ground stations
 * 
...
void transmit()
{
  ...
  Serial.println("Address Packet: ");
  for(int j=0;j<sizeof(satAddress); j++)
        { Serial.print(satAddress[j],HEX); Serial.print(" ");
        } Serial.println();
   Serial.println("Telemetry Packet: ");
  for(int j=0;j<sizeof(satTelemetry); j++)
          { Serial.print(satTelemetry[j],HEX); Serial.print(" ");
          } Serial.println();
