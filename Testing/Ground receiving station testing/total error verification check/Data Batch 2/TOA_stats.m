...
figure
plot(TOA_diff_no_spike(:,1)*1000,'lineWidth',1);
hold on; grid minor; xlim([0 length(TOA_diff(:,1))]);
plot(TOA_diff_no_spike(:,2)*1000,'lineWidth',1);
plot(TOA_diff_no_spike(:,3)*1000,'lineWidth',1);
plot(TOA_diff_no_spike(:,4)*1000,'lineWidth',1);
plot(TOA_diff_no_spike(:,5)*1000,'lineWidth',1);
ax=gca;ax.FontSize=16;
...
average_time(i) = (abs(TOA_diff_no_spike(i,1)) + abs(TOA_diff_no_spike(i,2)) + abs(TOA_diff_no_spike(i,3)) + abs(TOA_diff_no_spike(i,4)) )/4;
end
...
ax=gca;ax.FontSize=16;
title('Total error in address average timing measurement between 2 stations with no spikes','fontSize',24);
xlabel('Sample number (n)','fontSize',16);
ylabel('Time (ms)','fontSize',16);
